...
class CIFAR10:
    def __init__(self, data_path):
        ...
        for file_name in file_names:
            with open(data_path + file_name, mode='rb') as fin:
        ...
        fin = open(data_path + 'batches.meta',mode="rb")

        self.LABEL_NAMES = pickle.load(fin, encoding='bytes')[b'label_names']
        self.names = [x.decode('utf-8') for x in self.LABEL_NAMES]
        print(self.names)
    ...
    def show_img(self, idx):
        cv2.imshow(self.names[self.y[idx]], self.__prep_img(idx))
        cv2.waitKey(0)
    ...
    def show_examples(self):
        ...
        for i in range(5):
            for j in range(5):
                ...
                axes[i][j].set_title(self.names[self.y[rand]])
        plt.show()
...
dataset = CIFAR10('./image classification/cifar-10-batches-py/')
